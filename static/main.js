...
function formatStepContent(data, eventType) {
    // Special case for certain formats
...
function addVisualizationItem(data, sourceId = null) {
    ...
    const existingItem = workspace.querySelector(`[data-source="${data.id || sourceId}"]`);
    if (existingItem) {
        // If it already exists, don't add it again
        existingItem.classList.add('highlight');
        setTimeout(() => existingItem.classList.remove('highlight'), 2000);
        return;
    }
    
    // Create wrapper for the visualization (holds resize handles)
    const wrapper = document.createElement('div');
    wrapper.className = 'visualization-wrapper';
    wrapper.dataset.source = data.id || sourceId || `vis-${Date.now()}`;
    
    // Add resize handles to the wrapper
    ...
    if (data.type === 'table' && data.content && data.content.headers) {
        ...
        data.content.headers.forEach(header => {
            ...
            // Add the resize handle directly
            ...
            resizer.style.backgroundColor = '#0077cc';
        ...
        // Make the table headers resizable with a minimum width
        setTimeout(() => {
            ...
            tableHeaders.forEach(th => {
                ...
                grip.addEventListener('mousedown', function(e) {
                    ...
                    const moveHandler = function(e) {
                        th.style.width = Math.max(50, startWidth + e.pageX - startX) + 'px';
                    };
    ...
    // Make the wrapper draggable within the workspace
    ...
    wrapper.addEventListener('mousedown', function(e) {
        // Skip if clicking on resize handles or table column resizers
        if (e.target.classList.contains('resize-handle') || 
            e.target.classList.contains('column-resizer') || 
            e.target.tagName === 'TH' || 
            e.target.closest('th')) return;
    ...
    // Add resize functionality to the wrapper
    const resizeHandleElements = wrapper.querySelectorAll('.resize-handle');
    ...
    // Highlight the wrapper briefly to draw attention
...
// Helper function to convert MySQL/table output from tool results
function convertMySQLOutputToMarkdown(mysqlOutput) {
    if (!mysqlOutput) return null;
...
function setupUI() {
    ...
    styleElement.textContent = `
        ...
        /* Resize handle for visualization items - subtle version */
        .resize-handle {
            position: absolute;
        ...
        /* Make resize handles slightly visible on hover */
        .visualization-wrapper:hover .resize-handle:hover {
        ...
        .visualization-wrapper.highlight {
